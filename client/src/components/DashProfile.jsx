  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      if (data.secure_url) {
        ...
        // ✅ Update profilePicture separately
        const updatePicRes = await fetch(
        ...
        const updatedUser = await updatePicRes.json();

        if (!updatePicRes.ok) {
          dispatch(
            updateFailure("Failed to update user image", updatedUser.message)
          );
        } else {
          dispatch(updateSuccess(updatedUser)); // so redux state stays in sync
        }
      } else {
        dispatch(updateFailure("Cloudinary response error", data));
      }
    } catch (err) {
